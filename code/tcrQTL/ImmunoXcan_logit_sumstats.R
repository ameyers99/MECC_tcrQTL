...
option_list = list(
  ...
  make_option("--pe_filter", action="store", default=0.05, type='double', 
...
# Sample IDs 
all_samples <- fread(opt$sample_file, header = FALSE)
setnames(all_samples, "V1", "Sample")
  ...
  if (length(covs) > 0) {
   ...
   lr2_fit <- tryCatch(
    logistf(
      ...
      na.action = na.omit,
	  pl = FALSE,
	  control = logistf.control(maxit = 20, 
								maxhs = 0, 
								maxstep = 5, 
								lconv = 1e-04, 
								gconv = 1e-04, 
