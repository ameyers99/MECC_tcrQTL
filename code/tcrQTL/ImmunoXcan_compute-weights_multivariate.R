...
option_list = list(
  ...
  make_option("--jitter", action="store", default=NULL, type='double',
...
tcr <- (function(df) {
  ...
  nzp <- sapply(df[cols], function(x) mean(x > 0, na.rm = TRUE))
  keep <- nzp >= opt$pe_filter
  dropped <- cols[!keep]
  kept <- cols[keep]
  if (opt$verbose >= 1) {
    message(sprintf("Dropping %d / %d features (prevalence < %.3f)",
                    length(dropped), length(cols), opt$pe_filter))
    if (length(dropped) > 0) {
      message("Dropped features: ", paste(dropped, collapse = ", "))
    }
    message(sprintf("Remaining features: %d", length(kept)))
  }
  df[, c("IID", kept)]
})(tcr)
tcr.og <- tcr

if (ncol(tcr) > 2) { # Only run MV analysis if ≥2 features present (ncol includes IID)
...
if ( !is.na(opt$covar) ) {
  ...
  tcr[ , setdiff(names(tcr), "IID")] <- scale(tcr[ , setdiff(names(tcr), "IID")])
}
...
# - Map IDs
  ...
  m = match(rownames(genos_alt$bed), rownames(tcr))
  m.keep = !is.na(m)
  genos_alt$bed = genos_alt$bed[m.keep, ]
  tcr = tcr[m, ]
  tcr = tcr[m.keep, ]
...
common_ids <- rownames(geno_mat)[rownames(geno_mat) %in% common_ids]
...
for (trait in colnames(tcr)) {
  ...
  # Response vector
  tcr.vec <- colnames(tcr)

  # Save
  save(wgt.matrix, genotypes, cv.performance, N.tot, tcr.pe, tcr.vec,
...
cleanup()
  ...
  if ( opt$verbose >= 1 ) cat("<2 features pass prevalence filter, skipped\n")
  if ( opt$verbose >= 1 ) cat("Cleaning up\n")
  cleanup()
}
