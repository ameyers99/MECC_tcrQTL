...
option_list = list(
  ...
  make_option("--pe_filter", action="store", default=0.50, type='double', 
...
# Include all samples in the fam file, with expression value 0 if missing
tcr <- fam %>%
  select(IID) %>%
...
# NB, assumes prior QC (incl. MAF) processing
